...
function [polarization] = BasicNoiseVicsek(agents, agentSize, sizeOfGraph, radius, frames, speed, fps, eta)
   ...
   for i = 1:numOfAgents(2)
    ...
    % Remember, f1 is a vector pointing from the agent to the neighbor
     ...
     c = c + exp(-norm(coords)/(2 * sizeOfGraph));
      % This is no noise
       agents(3, i) = mod(sumOfDirections / numOfNeighbors + eta * rand() * pi * (1 - 2 * rand()), 2 * pi); 
